...
list(data, top) {
  ...
  const generateField = (obj) => {
    return Object.entries(obj).map(([key, value]) => `*${key}:* _${value}_`).join('\n');
  };
...
mapList(data, top) {
  ...
  const stringifyValue = (value) => {
    if (typeof value === 'object' && value !== null) {
      return Object.entries(value).map(([key, val]) => `*${key}:* ${stringifyValue(val)}`).join('\n');
    }
    return `_${value}_`;
  };
  const generateField = (v, index) => {
    const fields = Object.entries(v).map(([key, value]) => `*${key}:* ${stringifyValue(value)}`).join('\n');
...
transformText(input) {
  ...
  return input.split(linkPattern).map(part => {
    if (linkPattern.test(part)) {
      ...
      return part.split(atNumberPattern).map((subpart, index, arr) => {
