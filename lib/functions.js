...
export default {
  async api(endpoint, value, options = {}) {
    const {
        base = config.settings.restapi,
        paramName = typeof value === 'object' ? "" : value && value.startsWith("https://") ? "url" : "text",
        buffer = false,
        name = this.getRandom("be", 5)
    } = options;
  ...
  extractCategoriesFromSwitch(switchCode) {
    const regex = /case\s+["']([^"']+)["'](?:\s*:\s*case\s+["']([^"']+)["'])*\s*:\s*{[^]*?\/\/\s*([\w]+)(?=\s*(?:\n|$))/g;
    const categories = {};
    let match;
    while ((match = regex.exec(switchCode)) !== null) {
        const category = match[3];
        if (category.includes('->')) continue; // Category ignoring if there is a sign "->"
  ...
  limit(m, need) {
    ...
    if (user.limit < need) {
      m.reply(`Your limit is not enough to access the feature!\n\nA ${need} limit is required.\nYour limit: ${user.limit}`);
      return false;
    }
    user.limit -= need;
    m.reply(`Your limit used: ${need}.\n\nRemaining limit: ${user.limit}`);
    return true;
  },
  ...
  timer(ms) {
    ...
    const y = Math.floor(ms / 31536000000);
    const mo = Math.floor(ms / 2592000000) % 12;
    const d = Math.floor(ms / 86400000) % 30;
  ...
  card(options, url, body, footer, button, title, text, id) {
    if (!Array.isArray(options) || options.length === 0) {
        const buttons = Array.isArray(button) ? button.map(v => {
            const buttonTitle = title ? v[title] : v.title;
            const buttonText = text ? v[text] : v.text;
            const buttonId = id && id.startsWith(config.settings.prefix) 
                ? `${id.slice(0, id.indexOf(" "))} ${v[id.split(" ").pop()]}` 
                : id ? v[id] : v.id;
            if (buttonTitle && buttonText && buttonId) {
                return {
                    name: String(buttonTitle),
        ...
        return [{
            url: url || config.logo.thumb || "",
            body: body || "",
            footer: config.name.bot || "",
            buttons: buttons
        }];
    }
    return options.map((item, index) => {
        const itemUrl = item?.[url] 
            ? String(item[url]) 
            : (item?.url ? item.url : config.logo.thumb);
        const itemBody = item?.[body] 
            ? String(item[body]) 
            : (item?.title ? item.title : `*Hasil ke: ${index + 1}*`);
        const itemFooter = item?.[footer] 
            ? String(item[footer]) 
            : (item?.id ? String(item.id) : (item.desc ? item.desc : config.name.bot));
        const buttons = Array.isArray(button) ? button.map(v => {
            const buttonTitle = title ? v[title] : v.title;
            const buttonText = text ? v[text] : v.text;
            const buttonId = id && id.startsWith(config.settings.prefix) 
  ...
  async getFile(PATH, text = "") {
    ...
    let mime = /webp/.test(type.mime) ? "sticker/" : /audio/.test(type.mime) ? "audio/" : /video/.test(type.mime) ? "video/" : /image/.test(type.mime) ? "image/" : "bin/";
