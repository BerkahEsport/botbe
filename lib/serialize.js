...
export default async function serialize(sock, m, store, config, functions) {
	...
	if (m.message) {
			...
			m.cmd = m.body.trim().replace(config.settings.prefix || ".", "").split(/ +/)[0].toLowerCase();
			...
			if (m.key) {
				...
				m.pushName = sock.getName(m.sender);
	...
	m.reply = async function replyMessage(text = "", options = {}) {
		...
		if (/^https?:\/\//.test(text)) {
		...
		} else if (!!config.msg[text]) {
			return sock.reply(jid, config.msg[text], quoted, { ...options });
			...
			return sock.reply(jid, functions.format(text), m, { ...options});
		} else {
	...
	m.log = function logMessage(...data) {
		...
		data.forEach((item) => {
			...
			console.log(color + (typeof item === "object" ? functions.format(item) : item) + reset);
		});
	};	
	// Quoted message
	if (m.isQuoted) {
