...
export default async function serialize(sock, m, store, config, functions) {
	...
	if (m.message) {
        ...
        : m.type === "messageContextInfo" ? m.message?.listResponseMessage?.singleSelectReply?.selectedRowId || m.message?.buttonsResponseMessage?.selectedButtonId || await (JSON?.parse(m.message?.interactiveResponseMessage?.nativeFlowResponseMessage?.paramsJson).id) || ""
        : "";
