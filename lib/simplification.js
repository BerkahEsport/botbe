...
<============== CREDITS ==============>*/
import baileys, {prepareWAMessageMedia, jidNormalizedUser, generateWAMessageFromContent, toBuffer, jidDecode, downloadContentFromMessage, WA_DEFAULT_EPHEMERAL } from "baileys";
const {proto} = baileys
...
export default async function whatsapp(sock, store, config, functions) {
	function getContentType(object) {
		if (object) {
	...
	sock.reply = async (jid, text = "", quoted = "", options = {}) => {
		if (typeof text == "object") {
		   	return sock.sendMessage(jid, {
				...
				}, { ephemeralExpiration: WA_DEFAULT_EPHEMERAL || 604800, quoted, ...options });
			...
			contextInfo: {
				...
				externalAdReply: {
					...
					mediaType: 1,
					previewType: 5,
					renderLargerThumbnail: !!options.thumb ? true : false,
					thumbnail: !!options.thumb ? await (await functions.getFile(options.thumb)).data : fs.readFileSync("./src/thumbnail.jpg"),
	...
	sock.sendFile = async (jid, url, fileName = "", caption = "", quoted = "", options = {}) => {
		...
		if (options.asDocument) data = { 
			...
			contextInfo: {
				...
				externalAdReply: {
					title: config.name.bot || sock.user.name,
					body: !!options.body ? options.body : config.name.bot || sock.user.name, 
					mediaType: 1,
					previewType: 5, 
					renderLargerThumbnail: true,
					thumbnail: !!options.thumb ? await (await functions.fetchBuffer(options.thumb)).data : fs.readFileSync("./src/thumbnail.jpg"),
					sourceUrl: config.group.ofcid,
					showAdAttribution: !!options.isForwarded ? false : true
				}, ...options
			}
	}
	 else if (options.asSticker || /webp/.test(mime)) {
		let pathFile = await writeExif({ mimetype: mime, data: buffer }, { ...options }, fileName)
		...
		contextInfo: {
			forwardingScore: !!options.isForwarded ? functions.randomInt(10, 256) : false,
			isForwarded: !!options.isForwarded ? true : false,
			mentionedJid: !!options.mentions ? options.mentions : [jid, ...sock.parseMentions(caption)],
			externalAdReply: {
				title: config.name.bot || sock.user.name,
				body: !!options.body ? options.body : config.name.bot || sock.user.name, 
				mediaType: 1,
				previewType: !!options.large ? 5 : 1,
				renderLargerThumbnail: !!options.large ? true : false,
				thumbnail: !!options.thumb ? await (await functions.fetchBuffer(options.thumb)).data : fs.readFileSync("./src/thumbnail.jpg"),
				sourceUrl: config.group.ofcid,
				showAdAttribution: !!options.isForwarded ? false : true
			}, ...options
