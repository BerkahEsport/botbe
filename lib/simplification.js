...
export default async function whatsapp(sock, store, config, functions) {
	...
	sock.getName = (jid) => {
		...
		if (id.endsWith('g.us')) {
			...
			return metadata?.name || metadata?.verifiedName || metadata?.notify || jid.split("@")[0];
		}
	}
	...
	sock.sendContact = async(jid, number, quoted) => {
		const email = config.settings.email;
		let contacts;
		if (typeof number === "string") {
			const waId = number.endsWith("@s.whatsapp.net") ? number : `${number}@s.whatsapp.net`;
			const numberOnly = number.endsWith("@s.whatsapp.net") ? number.split`@`[0] : number;
			const displayName = await sock.getName(waId);
			contacts = [{
				displayName,
				vcard: `BEGIN:VCARD
VERSION:3.0
...
item1.TEL;waid=${numberOnly}:${waId}
item1.X-ABLabel:Nomor Creator Bot
item2.EMAIL;type=INTERNET:${email}
item2.X-ABLabel:Email Owner
item3.ADR:;;🇮🇩 Indonesia;;;;
item3.X-ABLabel:Region
item4.URL:https://tinyurl.com/berkahesportid
item4.X-ABLabel:Youtube
END:VCARD`}];
		} else {
		contacts = await Promise.all(number.map(async (v) => {
			const waId = v.endsWith("@s.whatsapp.net") ? v : `${v}@s.whatsapp.net`;
			const numberOnly = v.endsWith("@s.whatsapp.net") ? v.split`@`[0] : v;
			const displayName = await sock.getName(waId);
			return {
				displayName,
				vcard: `BEGIN:VCARD
...
END:VCARD`
		...
		mentions: quoted?.isGroup ? [sock.decodeJid(quoted?.participant)] : [sock.decodeJid(sock?.user?.id)],
		}, { ephemeralExpiration: quoted ? quoted.expiration : 10000000, quoted})
	}
	...
	// Button [contact me: wa.me/6289654279897] to unlock
	...
	// Carousel [contact me: wa.me/6289654279897] to unlock
